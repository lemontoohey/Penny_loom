...
import { STAGE3_QUIZ } from '../../../data/stage3Quiz';
...
export function ThreadingQuiz({ onComplete, onRetry }: { onComplete: (score: number) => void; onRetry: () => void }) {
  const { addXP, setChatOpen, setQuizScore, addBadge } = useGameStore();

  const total = STAGE3_QUIZ.length;
  ...
  const currentQ = STAGE3_QUIZ[qIndex];
  ...
  function pick(i: number) {
    ...
    if (correct) addXP(10, `Stage 3 quiz: ${currentQ.id}`);
    ...
    setTimeout(() => {
      ...
      if (qIndex < total - 1) {
        ...
        setAnswers((prev) => {
          ...
          setQuizScore(3, finalScore);
          if (finalScore >= PASS_THRESHOLD) addBadge('thread-master');
          setFinished(true);
          return prev;
        });
      }
    ...
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5 text-center">
        <div className="py-4">
          ...
          <h2 className="text-2xl font-bold text-loom-cream mb-1">{passed ? 'Thread master!' : 'Keep going!'}</h2>
          <p className="text-loom-muted text-sm mb-5">{score}/{total} correct · {passed ? 'Stage 3 complete!' : `Need ${PASS_THRESHOLD} to pass`}</p>
          ...
          {passed && (
            ...
            >
              <span>🪡</span>
              <span className="text-loom-gold text-sm font-semibold">Thread Master badge earned!</span>
            </motion.div>
          )}
            ...
            {STAGE3_QUIZ.map((q) => {
              ...
              return (
                <div key={q.id} className={`flex items-center gap-2 text-xs rounded-lg px-3 py-2 ${ok ? 'bg-loom-sage/10 text-loom-sage' : 'bg-red-900/20 text-red-300'}`}>
                  ...
                  <span className="text-[10px] opacity-70">{ok ? '+10 XP' : q.options[q.correct].slice(0, 22)}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="flex gap-3">
            ...
            {passed
              ? <Button onClick={() => onComplete(score)} fullWidth size="lg">Complete Stage 3 →</Button>
              : <Button onClick={() => setChatOpen(true)} variant="sage" fullWidth>Ask Wren</Button>}
          </div>
        </div>
      </motion.div>
    );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-loom-cream">Threading Quiz</h2>
